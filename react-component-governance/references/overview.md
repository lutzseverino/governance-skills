...
5. Read the matching role and pattern references when you need deeper guidance on the chosen classification.
6. Apply [Component Packaging Policy](./packaging.md) to decide how the chosen boundary appears in the filesystem.
7. Apply [Component Ownership And Sharing Policy](./ownership.md) to decide what stays local and what may move into broader shared scope.
8. Use [Component Boundary Cases](./boundary-cases.md) when a provider, screen, form coordinator, or loading/error shell does not fit cleanly at first glance.
